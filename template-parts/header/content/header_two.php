        ...
        <?php if (!empty(Egns_Helpers::egns_get_theme_option('header_two_menu_button')) && (Egns_Helpers::egns_get_theme_option('header_two_menu_button')   ==  1) && class_exists('CSF')) :  ?>
            <?php if (!empty(Egns_Helpers::egns_get_theme_option('header_two_button_url'))) :   ?>
                <div class="header-btn d-xl-block d-none">
                    <a href="<?php echo esc_url(Egns_Helpers::egns_get_theme_option('header_two_button_url')) ?>" class="button-hover-two"><?php echo esc_html(Egns_Helpers::egns_get_theme_option('header_two_button_text')) ?></a>
                </div>
            <?php endif ?>
        <?php endif ?>
        <div>
        ...
        <div class="container-fluid">
            <div class="row align-items-center">
                ...
                <div class="col-lg-6">
                    ...
                    <li class="citem" >
                        <?php if( !empty($egns_theme_options['header_two_sidebar_contact_number_two']) ) : ?>
                            <h6><?php echo esc_html('PHONE:') ?></h6>
                            <a href="tel:<?php echo esc_html($egns_theme_options['header_two_sidebar_contact_number_two'] ) ?>"><?php echo esc_html($egns_theme_options['header_two_sidebar_contact_number_two'] ) ?></a>
                        <?php endif ?>
                        </li>
                        <li class="citem">
                        <?php if( !empty($egns_theme_options['header_two_sidebar_contact_address_two']) ) : ?>
                            <h6><?php echo esc_html('ADDRESS:') ?></h6>
                            <p><?php echo esc_html($egns_theme_options['header_two_sidebar_contact_address_two'] ) ?></p>
                        <?php endif ?>
                        </li>
                        <li class="citem">
                        <?php if( !empty($egns_theme_options['header_two_sidebar_contact_email_two']) ) : ?>
                            <h6><?php echo esc_html('E-mail') ?></h6>
                            <a href="mailto:<?php echo esc_html($egns_theme_options['header_two_sidebar_contact_email_two'] ) ?>"><?php echo esc_html($egns_theme_options['header_two_sidebar_contact_email_two'] ) ?></a>
                        <?php endif ?>
                        </li>
                        <li class="citem">
                            <h6><?php echo esc_html('FOLLOW US:') ?></h6>
                            <ul class="social-icon">
                                <?php if( is_array( Egns_Helpers::egns_get_theme_option('header_two_sidebar_social_link') ) ) : ?>
                                    <?php foreach( Egns_Helpers::egns_get_theme_option('header_two_sidebar_social_link') as $row ) : ?>
                                        <li><a href="<?php echo esc_url( $row['header_two_sidebar_social_link_url'] ) ?>"><i class='<?php echo esc_attr( $row['header_two_sidebar_social_icon_class'] ) ?>'></i></a></li>
                                    <?php endforeach ?>
                                <?php endif ?>
                            </ul>
                        </li>
